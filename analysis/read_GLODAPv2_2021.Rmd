...
for (i_basin in unique(basinmask_5$basin)) {
  ...
  expocodes_subbasin <-
    inner_join(expocodes_subbasin_f, expocodes_subbasin_qc) %>%
    group_by(cruise_expocode) %>%
    summarise(n = min(n)) %>%
    ungroup()
  ...
  print(
  xover_cruise %>%
    filter(dateA > ymd("1990-01-01") &
             dateB > ymd("1990-01-01") &
             !is.na(offset_adj)) %>%
    ...
    map(
      ~ ggplot(data = .x,
               aes(dateA, dateB, col = offset_adj, size = n)) +
