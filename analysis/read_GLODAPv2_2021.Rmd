...
for (i_basin in unique(basinmask_5$basin)) {
  ...
  for (i_decade_start in seq(1990, 2010, 10)) {
    ...
    xover_NP_decade_stats <- xover_NP_decade %>%
      group_by(parameter, decade) %>%
      summarise(
    ...
    xover_NP_decade_stats <- xover_NP_decade_stats %>% 
      mutate(basin = i_basin,
             decade_start = i_decade_start)
    
    if (exists("xover_NP_decade_stats_all")) {
      xover_NP_decade_stats_all <-
        bind_rows(xover_NP_decade_stats_all, xover_NP_decade_stats)
    }
    
    if (!exists("xover_NP_decade_stats_all")) {
      xover_NP_decade_stats_all <- xover_NP_decade_stats
    }
    
    
    print(
      xover_NP_decade_stats %>%
    ...
    p_crossover_ts <- xover_NP_decade %>%
...
```{r full_decadal_crossover_analysis}

xover_NP_decade_stats_all %>%
  # mutate(decade_start = paste0(decade_start , "-", decade_start + 9)) %>%
