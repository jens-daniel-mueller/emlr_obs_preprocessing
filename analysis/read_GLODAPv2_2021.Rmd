...
for (i_expocodes_missing in expocodes_missing) {
  # i_expocodes_missing <- expocodes_missing[2]
  ...
  if (length(parameter_f9) > 0) {
    parameter_f9 <- parameter_f9 %>% str_c(.,"-f9")
  }
  ...
  hline_intercept <-
    tibble(parameter = unique(xover_cruise$parameter)) %>%
  ...
  p_crossover_ts <- xover_cruise %>%
    ...
    labs(title = paste(i_expocodes_missing,
                       "| ", parameter_check)) +
    theme(
