...
p_crossover_decadal <-
  ...
  #               filter(year(date_B) >= 2000, year(date_B) < 2010),
...
for (i_basin in unique(basinmask_5$basin)) {
  ...
  # extract cruise year from expocode
  ...
  for (i_cruise_expocode in expocodes_subbasin$cruise_expocode){
    ...
    # subset all crossover from cruise
...
for (i_basin in unique(basinmask_5$basin)) {
  ...
  print(
  xover_cruise %>%
    filter(date_A > ymd("1990-01-01") &
             date_B > ymd("1990-01-01") &
             !is.na(offset_adj)) %>%
