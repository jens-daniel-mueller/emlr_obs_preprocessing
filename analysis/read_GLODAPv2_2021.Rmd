...
```{r crossover_analysis_5_basin_decadal, fig.asp=1.2, results='asis'}

for (i_basin in unique(basinmask_5$basin)) {
  # i_basin <- unique(basinmask_5$basin)[3]
  
  
  NP_expocodes <- inner_join(GLODAP, basinmask_5) %>%
    filter(basin == i_basin)
  
  print(
  map +
    geom_tile(data = NP_expocodes %>% distinct(lon, lat),
              aes(lon, lat)) +
    labs(title = i_basin)
  )
  
  NP_expocodes_qc <- NP_expocodes %>%
  ...
  NP_expocodes <- inner_join(NP_expocodes_f, NP_expocodes_qc) %>%
    group_by(cruise_expocode) %>%
    summarise(n = min(n)) %>%
    ungroup()
  
  rm(NP_expocodes_f, NP_expocodes_qc)
  
  xover_NP <- xover %>%
    filter(
      cruise_A %in% NP_expocodes$cruise_expocode &
        cruise_B %in% NP_expocodes$cruise_expocode
    )
  ...
  for (decade_start in seq(1990, 2010, 10)) {
    # decade_start <- seq(1990, 2010, 10)[3]
    
    xover_NP_A <- xover_NP %>%
      filter(dateA >= decade_start & dateA < decade_start + 10)
    
    xover_NP_B <- xover_NP %>%
      filter(dateB >= decade_start & dateB < decade_start + 10)
    
    xover_NP_B_rev <- xover_NP_B %>%
      rename(
        cruise_A_back = cruise_A,
        cruise_A = cruise_B,
        dateA_back = dateA,
    ...
    xover_NP_decade <- bind_rows(xover_NP_A,
                          xover_NP_B_rev)
    
    rm(xover_NP_A, xover_NP_B, xover_NP_B_rev)
    
    
    xover_NP_decade <- left_join(xover_NP_decade,
                          NP_expocodes %>% rename(cruise_A = cruise_expocode,
                                                  n_A = n))
    xover_NP_decade <- left_join(xover_NP_decade,
                          NP_expocodes %>% rename(cruise_B = cruise_expocode,
                                                  n_B = n))
    
    xover_NP_decade <- xover_NP_decade %>%
      mutate(n = n_A + n_B) %>%
    ...
    print(
      xover_NP_decade %>%
        ...
        ungroup() %>%
        kable(caption = paste0(i_basin, " | ", decade_start, "s")) %>%
        kable_styling() %>%
        scroll_box(height = "300px")
    )
    
    
    p_crossover_ts <- xover_NP %>%
      ggplot(aes(dateB, offset)) +
      geom_hline(data = hline_intercept, aes(yintercept = intercept)) +
      geom_point(shape = 21) +
      scale_color_brewer(palette = "Set1") +
      facet_grid(parameter ~ ., scales = "free_y") +
      labs(title = paste0(i_basin, " | ", decade_start, "s")) +
      theme(
        legend.position = "bottom",
        legend.direction = "vertical",
        axis.title.x = element_blank()
    ...
    p_crossover_decadal <-
      ggplot() +
      geom_hline(data = hline_intercept, aes(yintercept = intercept)) +
      geom_violin(data = xover_NP_decade,
                  aes(x = decade, y = offset), fill = "gold") +
      geom_boxplot(data = xover_NP_decade,
                   aes(x = decade, y = offset),
                   width = 0.2) +
      facet_grid(parameter ~ ., scales = "free_y") +
      labs(title = "Decadal offsets") +
...
```{r crossover_analysis_North_Pacific_2010s, fig.asp=1.2}

NP_expocodes <- inner_join(GLODAP, basinmask_5) %>%
  filter(basin == "N_Pacific")
...
xover_NP <- xover %>% 
  filter(cruise_A %in% NP_expocodes$cruise_expocode &
         cruise_B %in% NP_expocodes$cruise_expocode)
...
xover_NP_2010_A <- xover_NP %>%
  filter(dateA >= 2010)

xover_NP_2010_B <- xover_NP %>%
  filter(dateB >= 2010)

xover_NP_2010_B_rev <- xover_NP_2010_B %>%
  rename(
    cruise_A_back = cruise_A,
    cruise_A = cruise_B,
    dateA_back = dateA,
...
xover_NP_2010 <- bind_rows(xover_NP_2010_A,
                           xover_NP_2010_B_rev)


xover_NP_2010 <- left_join(
  xover_NP_2010,
  NP_expocodes %>% rename(cruise_A = cruise_expocode,
                          n_A = n))
xover_NP_2010 <- left_join(
...
xover_NP_2010 <- xover_NP_2010 %>% 
  mutate(n = n_A + n_B) %>% 
  select(-starts_with("n_"))

xover_NP_2010_decade <- xover_NP_2010 %>%
    mutate(decade = cut(
      dateB,
      c(1989, 1999, 2009, 2019),
      labels = c("1989-1999", "2000-2009", "2010-2019")
...
p_crossover_ts <- xover_NP_2010 %>%
  ggplot(aes(dateB, offset)) +
  geom_hline(data = hline_intercept, aes(yintercept = intercept)) +
  geom_point(shape = 21) +
  scale_color_brewer(palette = "Set1") +
  facet_grid(parameter ~ ., scales = "free_y") +
  labs(title = "Crossover with 2010s") +
  theme(
    legend.position = "bottom",
    legend.direction = "vertical",
    axis.title.x = element_blank()
...
p_crossover_decadal <-
  ggplot() +
  geom_hline(data = hline_intercept, aes(yintercept = intercept)) +
  geom_violin(data = xover_NP_2010_decade,
               aes(x = decade, y = offset), fill="gold") +
  geom_boxplot(data = xover_NP_2010_decade,
               aes(x = decade, y = offset),
               width = 0.2) +
  facet_grid(parameter ~ ., scales = "free_y") +
  labs(title = "Decadal offsets") +
...
```{r crossover_analysis_North_Pacific_2000s, fig.asp=1.2}

xover_NP_2000_A <- xover_NP %>%
  filter(dateA >= 2000 & dateA < 2010)

xover_NP_2000_B <- xover_NP %>%
  filter(dateB >= 2000 & dateB < 2010)

xover_NP_2000_B_rev <- xover_NP_2000_B %>%
  rename(
    cruise_A_back = cruise_A,
    cruise_A = cruise_B,
    dateA_back = dateA,
...
xover_NP_2000 <- bind_rows(xover_NP_2000_A,
                           xover_NP_2000_B_rev)

xover_NP_2000 <- left_join(
  xover_NP_2000,
  NP_expocodes %>% rename(cruise_A = cruise_expocode,
                          n_A = n))
xover_NP_2000 <- left_join(
...
xover_NP_2000 <- xover_NP_2000 %>% 
  mutate(n = n_A + n_B) %>% 
  select(-starts_with("n_"))

xover_NP_2000_decade <- xover_NP_2000 %>%
    mutate(decade = cut(
      dateB,
      c(1989, 1999, 2009, 2019),
      labels = c("1989-1999", "2000-2009", "2000-2019")
    )) %>%
    filter(!is.na(decade),
           !is.na(offset_adj),
           !is.na(offset)) %>% 
...
p_crossover_ts <- xover_NP_2000 %>%
  ggplot(aes(dateB, offset)) +
  geom_hline(data = hline_intercept, aes(yintercept = intercept)) +
  geom_point(shape = 21) +
  scale_color_brewer(palette = "Set1") +
  facet_grid(parameter ~ ., scales = "free_y") +
  labs(title = "Crossover with 2000s") +
  theme(
    legend.position = "bottom",
    legend.direction = "vertical",
    axis.title.x = element_blank()
...
p_crossover_decadal <-
  ggplot() +
  geom_hline(data = hline_intercept, aes(yintercept = intercept)) +
  geom_violin(data = xover_NP_2000_decade,
               aes(x = decade, y = offset), fill="gold") +
  geom_boxplot(data = xover_NP_2000_decade,
               aes(x = decade, y = offset),
               width = 0.2) +
  facet_grid(parameter ~ ., scales = "free_y") +
  labs(title = "Decadal offsets") +
...
```{r CANYON-B_vs_observationd, fig.asp=0.4, eval=FALSE}
