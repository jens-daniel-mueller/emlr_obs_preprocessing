...
m_xover_reverse <- function(df) {
  df_rev <- df %>%
    ...
    mutate(
      ...
      offset_adj = if_else(
        parameter %in% c("salinity", "talk", "tco2",
                         "cstar_total", "cstar_phosphate", "cstar_talk", "cstar_tco2"),
        -offset_adj,
        1 / offset_adj
      )
    )
...
xover_basin %>%
  ...
  mutate(offset_adj = cut(offset_adj, c(-Inf, -5, -2, 2, 5, Inf))) %>%
  group_split(basin, parameter) %>%
  # head(1) %>%
  map(
    ~ ggplot(data = .x,
...
xover_basin_annual %>%
  mutate(offset_adj_weighted_mean = cut(offset_adj_weighted_mean, c(-Inf, -5, -2, 2, 5, Inf))) %>%
  group_split(basin, parameter) %>%
  head(1) %>%
  map(
    ~ ggplot(data = .x,
...
```{r crossover_analysis_5_basin_decadal, fig.asp=1, results='asis'}
...
```{r xover_cruise_mean_by_cruise_size, fig.asp=1}
...
```{r xover_cruise_mean_by_offset, fig.asp=1, eval=FALSE}
