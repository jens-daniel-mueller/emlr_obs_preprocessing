...
loss_qc_cruise <- loss_qc %>%
  ...
  mutate(n_total = qc_0 + qc_1,
         category = if_else(qc_0 <= 0.1 * (n_total), "OK", "loss")) %>%
  ...
  mutate(rank_n_total = rank(-n_total)) %>%
  ungroup() %>% 
...
loss_qc_cruise %>%
  filter(rank_n_total <= 20) %>%
  group_split(basin_AIP) %>%
  # head(1) %>%
  map(
    ~ ggplot(data = .x,
             aes(rank_n_total, n_total, fill = as.factor(loss))) +
...
loss_qc_cruise %>% 
  filter(loss > 0, rank_n_total <= 20) %>% 
  select(basin_AIP, decade, parameter_class, rank_n_total, cruise_expocode) %>% 
  arrange(basin_AIP, decade, parameter_class, rank_n_total) %>% 
  kable() %>% 
  kable_styling()
...
loss_f_cruise <- loss_f %>%
  ...
  mutate(n_total = f_0 + f_2,
         category = if_else(f_0 <= 0.1 * (n_total), "OK", "loss")) %>%
  mutate(parameter_class = if_else(
    parameter %in% c("tco2", "talk", "phosphate"),
    "target",
    "predictor"
...
IO_CRM_offset %>%
  ...
  geom_hline(data = IO_CRM_offset_mean,
             aes(yintercept = offset_mean)) +
  geom_hline(
    data = IO_CRM_offset_mean,
  ...
  geom_point(aes(start_date, offset, fill = batch),
             shape = 21,
             size = 2) +
  facet_wrap( ~ parameter)
