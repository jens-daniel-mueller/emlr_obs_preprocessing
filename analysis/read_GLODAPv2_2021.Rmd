...
```{r crossover_analysis_5_basin_decadal, fig.asp=2, results='asis'}
...
```{r xover_cruise_mean_by_cruise_size, fig.asp=2}
...
```{r NPO_2010_decadal_crossover, fig.asp=1.2}

parameter_check <- c("cstar_total_phosphate",
                          "cstar_tco2",
                          "cstar_talk",
                          "nitrate",
                          "phosphate",
...
xover_cruise_decade_all_NPO_2010 <- 
  ...
  mutate(country = str_sub(cruise_A, 1, 2)) %>% 
...
GLODAP_adjustments_long %>%
  ...
  mutate(date = ymd(str_sub(cruise_expocode, start = 5)),
         country = str_sub(cruise_expocode, 1, 2)) %>%
  ggplot(aes(date,
             adjustment,
             fill = country)) +
  geom_hline(data = hline_intercept_NPO_2010, aes(yintercept = intercept)) +
...
JMA_adjustments_long <- JMA_adjustments %>%
  select(cruise_expocode = expocode,
...
```{r xover_cruise_mean_by_cruise_size_PO, fig.asp= 0.5}
...
```{r xover_decadal_averages, fig.asp=2}
...
```{r mean_decadal_cstar_xover_offsets, fig.asp=1}
...
```{r GLODAP_RG_parameter_offsets_IO, fig.asp=1}
...
```{r test_xover_averaging, eval=FALSE}
...
```{r xover_adj_Knorr_data, fig.asp=1.2}
