...
```{r crossover_analysis_data, fig.asp=1}
...
hline_intercept <- tibble(parameter = unique(xover_IO_1990$parameter)) %>%
...
p_crossover_decadal <-
  ...
  geom_violin(data = xover_IO_1990 %>%
               ...
               aes(x = "2000s", y = offset), fill="gold") +
  geom_boxplot(data = xover_IO_1990 %>%
                 filter(year(dateB) >= 2000, year(dateB) < 2010),
               aes(x = "2000s", y = offset),
               width = 0.2) +
  geom_violin(data = xover_IO_1990 %>%
                 filter(year(dateB) >= 2010, year(dateB) < 2020),
               aes(x = "2010s", y = offset), fill="gold") +
  geom_boxplot(data = xover_IO_1990 %>%
                 filter(year(dateB) >= 2010, year(dateB) < 2020),
               aes(x = "2010s", y = offset),
               width = 0.2) +
  facet_grid(parameter ~ ., scales = "free_y") +
...
p_crossover_decadal <-
  ...
  geom_violin(data = xover_IO_1990 %>%
                 filter(year(dateB) >= 2000, year(dateB) < 2010),
               aes(x = "2000s", y = offset_adj), fill="gold") +
  geom_boxplot(data = xover_IO_1990 %>%
                 filter(year(dateB) >= 2000, year(dateB) < 2010),
               aes(x = "2000s", y = offset_adj),
               width = 0.2) +
  geom_violin(data = xover_IO_1990 %>%
                 filter(year(dateB) >= 2010, year(dateB) < 2020),
               aes(x = "2010s", y = offset_adj), fill="gold") +
  geom_boxplot(data = xover_IO_1990 %>%
                 filter(year(dateB) >= 2010, year(dateB) < 2020),
               aes(x = "2010s", y = offset_adj),
               width = 0.2) +
  facet_grid(parameter ~ ., scales = "free_y") +
  labs(title = "Decadal offset_adjs") +
  theme(axis.title.x = element_blank())
...
p_adjustment_A_ts <- xover_IO_1990 %>%
  distinct(
    cruise_A,
    cruise_B,
    first_station_A,
  ...
  ggplot(aes(dateB, adjustment_A, col = RV)) +
  geom_hline(data = hline_intercept, aes(yintercept = intercept)) +
  geom_point() +
  scale_color_brewer(palette = "Set1") +
  facet_grid(parameter ~ ., scales = "free_y") +
  labs(title = "Crossover with cruises 316N199XXXXX from IO") +
  theme(legend.position = "left",
        axis.title.x = element_blank())

p_adjustment_A_ts

p_adjustment_B_ts <- xover_IO_1990 %>%
  distinct(
    cruise_A,
    cruise_B,
    first_station_B,
  ...
  ggplot(aes(dateB, adjustment_B, col = RV)) +
  geom_hline(data = hline_intercept, aes(yintercept = intercept)) +
  geom_point() +
  scale_color_brewer(palette = "Set1") +
  facet_grid(parameter ~ ., scales = "free_y") +
...
rm(p_adjustment_A_ts, p_adjustment_B_ts)
