...
```{r function_extract_xover_single_cruise}


xover_cruise_extractation <- function (df, expocode) {
  xover_cruise_A <- df %>%
...
for (i_expocodes_missing in expocodes_missing) {
  ...
  xover_cruise <- xover_cruise_extractation(
    df = xover,
...
xover_IO_1990 <- xover_cruise_extractation(df = xover,
...
Here, I filter all crossover and use only those were both cruises covered the subbasin of interest.
...
### Cruise-by-cruise full

```{r xover_cruise_by_cruise_data_prep}
...
xover_basin %>%
  filter(!is.na(offset_adj),
         ...
         parameter_coverage == "full") %>%
  mutate(offset_adj = cut(offset_adj, 1 + c(-Inf, -5, -2, -1, 1, 2, 5, Inf) /
                            100)) %>%
  group_split(basin, parameter) %>%
  head(1) %>%
  map(
...
### Cruise-by-cruise partial

```{r xover_cruise_by_cruise_partial}
...
```{r crossover_analysis_5_basin_decadal, fig.asp=1.2, results='asis', eval=FALSE}
...
for (i_basin in unique(basinmask_5$basin)) {
  ...
  for (i_cruise_expocode in expocodes_subbasin$cruise_expocode){
    ...
    xover_cruise <- xover_cruise %>% 
      group_by(cruise_A, cruise_B, date_A, date_B, parameter) %>% 
      summarise(offset = mean(offset, na.rm = TRUE),
                offset_adj = mean(offset_adj, na.rm = TRUE)) %>% 
      ungroup()
    
    # join with number of samples from cruise B
    ...
    xover_cruise_stats <- xover_cruise %>%
      group_by(parameter, date_A, decade) %>%
      summarise(
  ...
  # loop over decades
  # Note this starts from the original crossover data for the subbasin
  for (i_decade_start in seq(1990, 2010, 10)) {
    ...
    # filter cruises that took place in decade
    xover_subbasin_A <- xover_subbasin %>%
      filter(year(date_A) >= i_decade_start &
               year(date_A) < i_decade_start + 10)
    
    xover_subbasin_B <- xover_subbasin %>%
      filter(year(date_B) >= i_decade_start & 
               year(date_B) < i_decade_start + 10)
    ...
    # assign number of observations to both cruises
    ...
    # but cruise B date into decades
    xover_subbasin_decade <- xover_subbasin_decade %>%
      mutate(decade = m_grid_decade(year(date_B))) %>%
      filter(!is.na(decade),
             !is.na(offset_adj),
...
rm(xover_subbasin, xover_cruise_stats, xover_subbasin_decade, xover_subbasin_decade_stats)

```

### Decadal averages

```{r full_decadal_crossover_analysis, eval=FALSE}
...
```{r cruise_mean_crossover_analysis, eval=FALSE}
...
```{r cruise_crossover_analysis, eval=FALSE}
...
```{r cruise_mean_crossover_analysis_NP, eval=FALSE}
