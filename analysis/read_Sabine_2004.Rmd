...
```{r read_G19_cant_file, eval=FALSE}
...
```{r merge_cant_data_sets, eval=FALSE}

tcant_inv <- full_join(tcant_inv_G19 %>% mutate(estimate = "G19"),
                      tcant_inv_S04 %>% mutate(estimate = "S04")) %>% 
  select(-c(tcant_inv, eras))

rm(tcant_inv_G19, tcant_inv_S04)
...
```{r tcant_inv_comparison, fig.asp=1, eval=FALSE}
 
map +
  geom_raster(data = tcant_inv,
...
```{r calculate_global_inventory, eval=FALSE}
...
```{r calculate_tcant_inv_ratio, eval=FALSE}

tcant_inv_wide <- tcant_inv %>%
  pivot_wider(values_from = c(cant_pos_inv, cant_pos_inv_grid),
              names_from = estimate)

tcant_inv_wide <- tcant_inv_wide %>% 
  drop_na() %>% 
...
tcant_inv_rel <- tcant_inv_wide %>% 
  pivot_longer(cols = c(G19_rel, S04_rel),
                        names_to = "estimate",
                        values_to = "tcant_inv_rel")

```



```{r tcant_inv_rel_maps, fig.asp=1, eval=FALSE}

map +
  geom_raster(data = tcant_inv_rel,
              aes(lon, lat, fill = tcant_inv_rel*100)) +
  scale_fill_viridis_c() +
  facet_wrap( ~ estimate, ncol = 1) +
  theme(
    axis.title = element_blank(),
...
```{r tcant_inv_ratio_map, fig.asp=0.6, eval=FALSE}

map +
  geom_contour_filled(data = tcant_inv_wide %>%
